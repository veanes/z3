...
;; Idiom: 'is NOT exactly a [0-9]-code [0-9]{3}:[0-9]{3}' (negative lookaround over a
;;   counted field; forces reasoning about the COMPLEMENT of a bounded loop).
;; Vars: x, y ; layout xyx
;; Query: x in [0-9]+ (|x|>=3), y in [0-9]* ; NOT ( xyx in [0-9]{3}":"[0-9]{3} )
;; Status: SAT -- witness x="000", y="" : all-[0-9] word has no ":", so it is
...
(define-fun code () (RegEx String)   ;; [0-9]{3} ":" [0-9]{3}
  (re.++ ((_ re.loop 3 3) (re.range "0" "9")) (re.++ (str.to_re ":") ((_ re.loop 3 3) (re.range "0" "9")))))
