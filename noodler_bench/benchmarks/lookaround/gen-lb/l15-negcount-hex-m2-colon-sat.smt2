...
;; Idiom: 'is NOT exactly a [0-9a-f]-code [0-9a-f]{2}:[0-9a-f]{2}' (negative lookaround over a
;;   counted field; forces reasoning about the COMPLEMENT of a bounded loop).
;; Vars: x, y ; layout xyx
;; Query: x in [0-9a-f]+ (|x|>=2), y in [0-9a-f]* ; NOT ( xyx in [0-9a-f]{2}":"[0-9a-f]{2} )
;; Status: SAT -- witness x="aa", y="" : all-[0-9a-f] word has no ":", so it is
...
(define-fun code () (RegEx String)   ;; [0-9a-f]{2} ":" [0-9a-f]{2}
  (re.++ ((_ re.loop 2 2) (re.union (re.range "0" "9") (re.range "a" "f"))) (re.++ (str.to_re ":") ((_ re.loop 2 2) (re.union (re.range "0" "9") (re.range "a" "f"))))))
