...
;; Idiom: (?<!in) ... [a-z]{2}$  with the leading token forced into [0-9].
;;   language = ~(Sigma* "in") . [a-z]{2}
;; Vars: x, y ; layout xyyx (ends in x)
;; Query: x in [0-9]+ , others in Sigma+ ; xyyx in ~(Sigma* "in") . [a-z]{2}
...
(define-fun lang () (RegEx String)   ;; ~(Sigma* ctx) . [a-z]{2}
  (re.++ (re.comp (re.++ (re.* re.allchar) (str.to_re "in"))) ((_ re.loop 2 2) (re.range "a" "z"))))
