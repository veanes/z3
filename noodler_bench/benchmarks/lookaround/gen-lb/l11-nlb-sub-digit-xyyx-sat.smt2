...
;;   (?<!sub) [0-9]+   ==   ~(Sigma* "sub") . [0-9]+
;; Vars: x, y ; layout xyyx
;; Query: all vars in Sigma+ ; xyyx in ~(Sigma* "sub") . [0-9]+
;; Status: SAT -- witness all vars="0" (split u="" not ending in "sub", v=word in [0-9]+).
;; Stresses: rcat(compl(sigma(Sigma* "sub")), [0-9]+) in seq_split.
...
(define-fun lang () (RegEx String)   ;; ~(Sigma* ctx) . [0-9]+
  (re.++ (re.comp (re.++ (re.* re.allchar) (str.to_re "sub"))) (re.+ (re.range "0" "9"))))
