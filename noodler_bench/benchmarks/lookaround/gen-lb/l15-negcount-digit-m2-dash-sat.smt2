...
;; Idiom: 'is NOT exactly a [0-9]-code [0-9]{2}-[0-9]{2}' (negative lookaround over a
;;   counted field; forces reasoning about the COMPLEMENT of a bounded loop).
;; Vars: x, y ; layout xyx
;; Query: x in [0-9]+ (|x|>=2), y in [0-9]* ; NOT ( xyx in [0-9]{2}"-"[0-9]{2} )
;; Status: SAT -- witness x="00", y="" : all-[0-9] word has no "-", so it is
...
(define-fun code () (RegEx String)   ;; [0-9]{2} "-" [0-9]{2}
