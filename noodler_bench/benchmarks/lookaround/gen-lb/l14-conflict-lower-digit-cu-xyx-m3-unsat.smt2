...
;; Idiom: (?<!cu) ... [0-9]{3}$  with the leading token forced into [a-z].
;;   language = ~(Sigma* "cu") . [0-9]{3}
;; Vars: x, y ; layout xyx (ends in x)
;; Query: x in [a-z]+ , others in Sigma+ ; xyx in ~(Sigma* "cu") . [0-9]{3}
...
(define-fun lang () (RegEx String)   ;; ~(Sigma* ctx) . [0-9]{3}
  (re.++ (re.comp (re.++ (re.* re.allchar) (str.to_re "cu"))) ((_ re.loop 3 3) (re.range "0" "9"))))
