...
;; Idiom: (?<!left) ... [A-Z]{3}$  with the leading token forced into [0-9].
;;   language = ~(Sigma* "left") . [A-Z]{3}
;; Vars: x, y ; layout xyx (ends in x)
;; Query: x in [0-9]+ , others in Sigma+ ; xyx in ~(Sigma* "left") . [A-Z]{3}
...
(define-fun lang () (RegEx String)   ;; ~(Sigma* ctx) . [A-Z]{3}
  (re.++ (re.comp (re.++ (re.* re.allchar) (str.to_re "left"))) ((_ re.loop 3 3) (re.range "A" "Z"))))
