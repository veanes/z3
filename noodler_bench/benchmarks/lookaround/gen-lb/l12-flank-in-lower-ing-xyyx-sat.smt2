...
;; Idiom: (?<!in) [a-z]+ (?!ing)  ==  ~(Sigma* "in") . [a-z]+ . ~("ing" Sigma*)
;;   (cf. corpus '(?<!...)University(?! Road)': complement on BOTH sides of a core).
;; Vars: x, y ; layout xyyx
;; Query: all vars in Sigma+ ; xyyx in ~(Sigma* "in") . [a-z]+ . ~("ing" Sigma*)
;; Status: SAT -- witness all vars="a" (u="" not ending "in", core in [a-z]+, w="" not starting "ing").
;; Stresses: compl+rcat AND compl+lcat around a core (two-sided splitting).
;; Source: mined neg-lookbehind + neg-lookahead idiom
;;==========================================================================
(define-fun lang () (RegEx String)   ;; ~(Sigma* "in") . [a-z]+ . ~("ing" Sigma*)
  (re.++ (re.comp (re.++ (re.* re.allchar) (str.to_re "in"))) (re.++ (re.+ (re.range "a" "z")) (re.comp (re.++ (str.to_re "ing") (re.* re.allchar))))))
