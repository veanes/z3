...
;; Idiom: 'is NOT exactly a [a-z]-code [a-z]{3}_[a-z]{3}' (negative lookaround over a
;;   counted field; forces reasoning about the COMPLEMENT of a bounded loop).
;; Vars: x, y ; layout xyx
;; Query: x in [a-z]+ (|x|>=3), y in [a-z]* ; NOT ( xyx in [a-z]{3}"_"[a-z]{3} )
;; Status: SAT -- witness x="aaa", y="" : all-[a-z] word has no "_", so it is
...
(define-fun code () (RegEx String)   ;; [a-z]{3} "_" [a-z]{3}
  (re.++ ((_ re.loop 3 3) (re.range "a" "z")) (re.++ (str.to_re "_") ((_ re.loop 3 3) (re.range "a" "z")))))
