...
;; Idiom: 'is NOT exactly a [0-9a-f]-code [0-9a-f]{4}_[0-9a-f]{4}' (negative lookaround over a
;;   counted field; forces reasoning about the COMPLEMENT of a bounded loop).
;; Vars: x, y ; layout xyx
;; Query: x in [0-9a-f]+ (|x|>=4), y in [0-9a-f]* ; NOT ( xyx in [0-9a-f]{4}"_"[0-9a-f]{4} )
;; Status: SAT -- witness x="aaaa", y="" : all-[0-9a-f] word has no "_", so it is
...
(define-fun code () (RegEx String)   ;; [0-9a-f]{4} "_" [0-9a-f]{4}
  (re.++ ((_ re.loop 4 4) (re.union (re.range "0" "9") (re.range "a" "f"))) (re.++ (str.to_re "_") ((_ re.loop 4 4) (re.union (re.range "0" "9") (re.range "a" "f"))))))
