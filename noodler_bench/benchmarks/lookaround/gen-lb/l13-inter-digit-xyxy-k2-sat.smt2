...
;; Idiom: 2 simultaneous negative lookbehinds  AND_i (?<!ab|cd) [0-9]+ .
;;   == intersection_i ~(Sigma* "ab") . [0-9]+   (each split-set crossed with the next).
;; Vars: x, y ; layout xyxy
;; Query: all vars in Sigma+ ; xyxy in AND of 2 complement-concats
...
;;==========================================================================
(define-fun lang () (RegEx String)   ;; intersection of 2 neg-lookbehinds, each . [0-9]+
  (re.inter (re.++ (re.comp (re.++ (re.* re.allchar) (str.to_re "ab"))) (re.+ (re.range "0" "9"))) (re.++ (re.comp (re.++ (re.* re.allchar) (str.to_re "cd"))) (re.+ (re.range "0" "9")))))
