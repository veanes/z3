...
;; Idiom: (?<!cu) [0-9]+ (?!ed)  ==  ~(Sigma* "cu") . [0-9]+ . ~("ed" Sigma*)
;;   (cf. corpus '(?<!...)University(?! Road)': complement on BOTH sides of a core).
;; Vars: x, y ; layout xyyx
;; Query: all vars in Sigma+ ; xyyx in ~(Sigma* "cu") . [0-9]+ . ~("ed" Sigma*)
;; Status: SAT -- witness all vars="0" (u="" not ending "cu", core in [0-9]+, w="" not starting "ed").
;; Stresses: compl+rcat AND compl+lcat around a core (two-sided splitting).
;; Source: mined neg-lookbehind + neg-lookahead idiom
;;==========================================================================
(define-fun lang () (RegEx String)   ;; ~(Sigma* "cu") . [0-9]+ . ~("ed" Sigma*)
  (re.++ (re.comp (re.++ (re.* re.allchar) (str.to_re "cu"))) (re.++ (re.+ (re.range "0" "9")) (re.comp (re.++ (str.to_re "ed") (re.* re.allchar))))))
