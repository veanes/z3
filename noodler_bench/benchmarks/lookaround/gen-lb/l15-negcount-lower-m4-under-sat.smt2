...
;; Idiom: 'is NOT exactly a [a-z]-code [a-z]{4}_[a-z]{4}' (negative lookaround over a
;;   counted field; forces reasoning about the COMPLEMENT of a bounded loop).
;; Vars: x, y ; layout xyx
;; Query: x in [a-z]+ (|x|>=4), y in [a-z]* ; NOT ( xyx in [a-z]{4}"_"[a-z]{4} )
;; Status: SAT -- witness x="aaaa", y="" : all-[a-z] word has no "_", so it is
...
(define-fun code () (RegEx String)   ;; [a-z]{4} "_" [a-z]{4}
  (re.++ ((_ re.loop 4 4) (re.range "a" "z")) (re.++ (str.to_re "_") ((_ re.loop 4 4) (re.range "a" "z")))))
