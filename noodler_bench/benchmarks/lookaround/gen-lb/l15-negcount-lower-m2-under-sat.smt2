...
;; Idiom: 'is NOT exactly a [a-z]-code [a-z]{2}_[a-z]{2}' (negative lookaround over a
;;   counted field; forces reasoning about the COMPLEMENT of a bounded loop).
;; Vars: x, y ; layout xyx
;; Query: x in [a-z]+ (|x|>=2), y in [a-z]* ; NOT ( xyx in [a-z]{2}"_"[a-z]{2} )
;; Status: SAT -- witness x="aa", y="" : all-[a-z] word has no "_", so it is
...
(define-fun code () (RegEx String)   ;; [a-z]{2} "_" [a-z]{2}
  (re.++ ((_ re.loop 2 2) (re.range "a" "z")) (re.++ (str.to_re "_") ((_ re.loop 2 2) (re.range "a" "z")))))
