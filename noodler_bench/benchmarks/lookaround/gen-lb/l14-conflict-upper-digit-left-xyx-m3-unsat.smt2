...
;; Idiom: (?<!left) ... [0-9]{3}$  with the leading token forced into [A-Z].
;;   language = ~(Sigma* "left") . [0-9]{3}
;; Vars: x, y ; layout xyx (ends in x)
;; Query: x in [A-Z]+ , others in Sigma+ ; xyx in ~(Sigma* "left") . [0-9]{3}
;; Status: UNSAT -- the word ends with [0-9]{3} so its last char is in [0-9];
...
(define-fun lang () (RegEx String)   ;; ~(Sigma* ctx) . [0-9]{3}
  (re.++ (re.comp (re.++ (re.* re.allchar) (str.to_re "left"))) ((_ re.loop 3 3) (re.range "0" "9"))))
