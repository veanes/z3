...
;; Query: all vars in Sigma+ ; xyyx in ~(Sigma* "cu") . [0-9]+
;; Status: SAT -- witness all vars="0" (split u="" not ending in "cu", v=word in [0-9]+).
;; Stresses: rcat(compl(sigma(Sigma* "cu")), [0-9]+) in seq_split.
;; Source: mined negative-lookbehind idiom
;;==========================================================================
(define-fun ctx () (RegEx String) (str.to_re "cu"))
(define-fun lang () (RegEx String)   ;; ~(Sigma* ctx) . [0-9]+
  (re.++ (re.comp (re.++ (re.* re.allchar) (str.to_re "cu"))) (re.+ (re.range "0" "9"))))
